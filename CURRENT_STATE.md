...
**现象**：交付版在物体边缘残留原物体轮廓（掩膜内边缘环与源帧梯度相关 +0.343，v1 仅 +0.058），
表现为"边缘一眼可见"，该项确实差于 v1。

**根因**：`_composite_source` 用**对称羽化** `alpha=GaussianBlur(mask)`，掩膜最外 1px 环 alpha 仅 0.621，
即 38% 源像素（被移除物体边缘）被混回，形成描边晕。差距不来自"没移除"，而来自合成把轮廓贴了回来。

**修复**：羽化只向掩膜外生效，掩膜内强制 alpha=1（`alpha=clip(blur(dilate(mask)))`，`alpha[mask]=1`）。

| 指标 | 修复前 | 修复后 |
| --- | --- | --- |
| 边界环物体残留相关（帧600） | +0.783 | **−0.073** |
| 全片掩膜内边缘环残留 | +0.343 | **+0.007**（优于 v1 +0.058） |
| 掩膜外背景偏差 | 0.14 | 0.47 |

副作用可忽略（未遮挡保真 2.61→3.20，填充锐度/闪烁不变）；复用已有 `stabilized` 生成重出视频，
无需重新推理；GLB 43,295 点、闪烁比 0.905、glitch 0。旧版留档 `snapshots/videos/background_video_edgehalo_backup.mp4`。

### 官方版本核验（2026-09-17）

`external/svor` 已**还原为官方 tarball 原版**（`/tmp/svor.tar.gz`，2026-05-20），全量 `diff -rq` 仅剩权重文件与 MODEL_CARD 差异，代码零改动。此前加的两处补丁（`--noise_file/--noise_offset` 与 `latents is None` 分支）已移除——它们经双窗 A/B 实测证伪（重叠区差异 9.03→10.03 变差，主导因素是 VACE 条件上下文而非初始噪声），且适配器内相关代码同步删除（59 项测试通过）。

用官方 Quick Test 素材（`samples/input/bmx-bumps_raw.mp4` 854×480×81，官方默认参数）复跑官方代码，与官方发布结果（`docs/assets/videos/result/bmx-bumps.mp4`，缩放到 320×180 对齐）定量对比：

| 指标 | 官方参考 | 官方代码本地复跑 |
| --- | --- | --- |
| 掩膜内改动（越高=移除越彻底） | 90.85 | 90.42 |
| 掩膜外改动（越低=背景保留越好） | 5.96 | **5.34** |
| 掩膜外结构相关（越高=真实结构保留越好） | 0.686 | **0.781** |
