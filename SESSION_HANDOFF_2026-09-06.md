...
- ⚠️ `outputs/001_sam31_slam/` **没有 `background_video.mp4`**：SLAM 路径从未跑过视频修复阶段（历次都 `--stop-after geometry`）。需要视频产物时去掉该参数完整运行（ProPainter 约 30-40 分钟）。
...
3. **补 SLAM 路径的视频产物**：`run --config configs/vggt_slam.yaml`（不带 --stop-after）得到 background_video.mp4，并用评估工具对比两后端的修复质量。
