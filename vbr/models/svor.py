...
adapter therefore synthesizes input/mask videos from the frame directories,
splits the clip into aligned chunks with overlap, runs SVOR per chunk, and
blend-stitches the chunk outputs before the final h264+aac encode.

Weights layout expected under `repo_dir/models/`:
- `Wan2.1-VACE-1.3B/`  base model (from Wan-AI/Wan2.1-VACE-1.3B)
...
class SVORAdapter:
    ...
    def run(self, video_path, frames_dir, masks_dir, output_path, fps):
        ...
        stitched = blend_stitch(chunk_outputs, raw_output, fps)
